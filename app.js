...
function log(message){
  try{if(Pip.log)Pip.log("IDLEFW: "+message,"pipco.txt");}catch(e){}
}
function loadConfig(){
  try{
...
E.defrag();
let config=loadConfig(),serviceFactory,idleService;
try{
  serviceFactory=eval(fs.readFileSync(basePath+"SERVICE.JS"));
  idleService=serviceFactory(config,fs,basePath,mesmetronPath,pipquariumPath,pipquariumSentinel,log);
}catch(e){
...
let providerNames=[],providerFiles=[];
try{
  let mesmetronTitle=eval(fs.readFileSync(mesmetronPath+"TITLE.JS"))(),
      mesmetronItems=mesmetronTitle&&mesmetronTitle.items?mesmetronTitle.items:[];
  for(let i=0;i<mesmetronItems.length;i++){
    if(mesmetronItems[i]&&mesmetronItems[i].name&&mesmetronItems[i].file){
...
function drawHeader(){
  ...
  try{
    let titleImage={
      width:176,height:98,bpp:1,transparent:0,
...
function drawText(y,text){
  h.setColor(3).setFontMonofonto16().setFontAlign(-1,-1).drawString(text,38,y);
}
...
function drawScroller(){
  ...
  let top=143,bottom=284,trackTop=153,trackBottom=274,max=Math.max(1,itemCount()-5);
  h.setColor(3)
  ...
  let height=Math.max(18,Math.floor((trackBottom-trackTop+1)*5/itemCount()));
...
function drawMenu(){
  ...
  if(menuPage===0){
    ...
    for(let i=0;i<5&&mainScroll+i<itemCount();i++){
      ...
      drawRow(142+i*30,itemLabel(itemIndex),itemIndex===mainSelection);
    }
    drawScroller();
    return;
  }
  ...
  if(submenuIndex===0){
    drawRow(126,"PIP-BOY 3000 Idle: "+(config.enabled?"ENABLED":"DISABLED"),submenuSelection===0);
...
return{
  ...
  remove:function(){
    ...
    if(configDirty){
      ...
      config.mesFile=selectedProvider?providerFiles[selectedProvider-1]:"";
    ...
    if(!idleService.enabled&&!idleService.providerEnabled)idleService.destroy();
