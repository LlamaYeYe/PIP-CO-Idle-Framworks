...
let c=0,page=0,mainSel=0,mainScroll=0,sub=-1,mesSel=0,dirty=0,previewSub=0;
...
function createService(cfg){
  ...
  function stop(redraw){
    ...
    if(redraw&&!S.runner){
      ...
      else setTimeout(function(){Pip.changeMenu();},0);
    }
  }
  ...
  function startBomb(){
    ...
    S.logo=undefined;
    E.defrag();
    try{S.sprite=eval(fs.readFileSync(B+"BOMB.JS"));}
    catch(e){stop(1);return 0;}
    ...
    if(!S.radioMuted){
      ...
      S.audioWatch=setInterval(function(){
        if(S.active&&!S.radioMuted&&S.ownAudio&&Pip.audioIsPlaying&&!Pip.audioIsPlaying()){
          Pip.audioStart(B+"FALLOUT.WAV",{repeat:true});
        }
      },10000);
    }
  ...
  function bombFrame(){"ram";
    if(!S.active||S.mode!==2)return;
    h.reset().setClipRect(0,0,479,319).clear(0);
    if(S.logoFrames>0){
      h.setColor(3).drawImage(S.logo,(480-S.logo.width)>>1,(320-S.logo.height)>>1);
      S.logoFrames--;
      h.flip();
    ...
    for(let i=0;i<14;i++){
      let dx=S.phase?3:2,dy=2;
      h.drawImage(S.sprite,S.x[i],S.y[i]);
      if(i===1||i===5||i===10){dx++;dy++;}
      S.x[i]-=dx;
      S.y[i]+=dy;
  ...
  function play(isPreview){
    ...
    E.defrag();
    try{
      S.logo={width:240,height:227,bpp:1,transparent:0,buffer:fs.readFileSync(B+"LOGO.BIN")};
    }catch(e){
      S.active=0;
      S.mode=0;
      if(S.pageSuspended)setTimeout(restorePage,0);
  ...
  function destroy(){
    stop(0);
    if(S.tick){clearInterval(S.tick);S.tick=undefined;}
    detachInputs();
...
if(cfg.mesEnabled&&cfg.mesFile){
  for(let i=0;i<mf.length;i++){
    if(mf[i]===cfg.mesFile){mesSel=i+1;break;}
  }
}
let S=createService(cfg);

function header(){
  h.clear().setColor(3).drawImage(titleImg,(480-176)>>1,6);
}
...
function scroller(){
  ...
  let top=143,bottom=284,trackTop=153,trackBottom=274,max=Math.max(1,count()-5);
  h.setColor(3)
    .drawLine(13,top+5,18,top).drawLine(18,top,23,top+5)
    .drawLine(13,bottom-5,18,bottom).drawLine(18,bottom,23,bottom-5)
    .drawLine(18,trackTop,18,trackBottom);
  let height=Math.max(18,Math.floor((trackBottom-trackTop+1)*5/count()));
  let y=trackTop+Math.floor(((trackBottom-trackTop+1)-height)*mainScroll/max);
...
function draw(){
  ...
  h.drawString(mesSel===sub?"ENABLED":"DISABLED",x+h.stringWidth(" Idle: "),108);
